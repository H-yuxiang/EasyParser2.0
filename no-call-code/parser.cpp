...
bool Parser::Analyse(std::vector<token> tk)
{
    ...
    // 输出控制
    std::ofstream outfile;
    outfile.open(PS_Right_Path, std::ios::out | std::ios::binary);
    // 分析
...
std::ofstream& Parser::show_tree(std::ofstream& outfile, treenode* root, int depth)
{
    // Display indentation
    ...
    if (root->child.size()) {
        for (size_t i = 0; i < root->child.size(); ++i) {
            show_tree(outfile, root->child[i], depth + 1);
        }
    }
...
void Parser::emit_tuple(int is_cmd_out)
{
    std::ofstream outfile;
    outfile.open(SM_QUADRUPLE_Path, std::ios::out | std::ios::binary);
    for (int i = 0; i < this->root->code.size(); ++i) {
        this->Q_ruple.push_back(this->root->code[i]);
        if (is_cmd_out) {
            // cmd
            std::cout << this->root->code[i].first << " ";
            std::cout << "(" << this->root->code[i].second.op << ", ";
            std::cout << this->root->code[i].second.arg1 << ", ";
            std::cout << this->root->code[i].second.arg2 << ", ";
            std::cout << this->root->code[i].second.result << ")\n";
        }
        // file
        outfile << this->root->code[i].first << " ";
        outfile << "(" << this->root->code[i].second.op << ", ";
        outfile << this->root->code[i].second.arg1 << ", ";
        outfile << this->root->code[i].second.arg2 << ", ";
        outfile << this->root->code[i].second.result << ")\n";
    }
    if (is_cmd_out) {
        // cmd
...
int Parser::dfs(treenode* root)
{
    ...
    if (root->value == this->Grammar_symbol_mp["赋值语句"]) {
        ...
        if (this->lookup(arg)) {
            ...
            return ERROR_NOT_FOUND;// 找不到变量 define
        }
    }
    // "Program"
    ...
    // "内部变量声明" eg. int a
        ...
        if (this->lookup(arg)) {
            ...
            return ERROR_REPEAT; // 变量重定义 define
        }
...
bool Parser::semanticAnalyse() {
    int res = this->dfs(this->root);
    std::ofstream outfile;
    outfile.open(PS_TREE_Path, std::ios::out | std::ios::binary);
    if (!outfile.is_open()) {
        std::cerr << "Error: Unable to open file." << std::endl;
        return false;  // Return the original (unmodified) ofstream object
    }
    if (res == SEMANTIC_RIGHT) {
        std::cout << "语义分析成功\n";
        show_tree(outfile, root);
//        this->gen()
        this->emit_tuple();
        return true;
    }
